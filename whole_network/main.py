...
def eval_genomes_threaded(genome, config, winner=0):
    ...
    while clock < max_time:
        ...
        #     #         genome.fitness -= 1e6
        #     # number of times next station is in previous_stations
        #     station_obj = station_objs[next_station_name]
        #     count = previous_stations.count(station_obj)
        #     num_plats = station_obj.num_platforms
        #     if count > num_plats + 1:
        #         genome.fitness -= 1e6
        # except IndexError:
        #     pass
        # update time
...
def run(config_path):
    ...
    load = 0
    if load:
        p = neat.Checkpointer.restore_checkpoint("neat-checkpoint-2697")
    else:
