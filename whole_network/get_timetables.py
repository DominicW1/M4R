...
def get_timetable(line, station_id, next_stop_id, intervalIds = None, route_number=0):
    k_id = 0
    kentish_town_id = station_dict["Kentish Town"]
    tufnell_park_id = station_dict["Tufnell Park"]
    archway_id = station_dict["Archway"]
    colindale_id = station_dict["Colindale"]
    burnt_oak_id = station_dict["Burnt Oak"]
    hendon_central_id = station_dict["Hendon Central"]
    if station_id == kentish_town_id:
        if next_stop_id == tufnell_park_id:
            station_id = tufnell_park_id
            next_stop_id = archway_id
            k_id = -2
        else:
            station_id = archway_id
            next_stop_id = tufnell_park_id
            k_id = 2
    if station_id == colindale_id:
        if next_stop_id == burnt_oak_id:
            with open("Colindale_to_Burnt_Oak_Timetable.json", "r") as f:
                json_data = json.load(f)
        else:
            with open("Colindale_to_Hendon_Central_Timetable.json", "r") as f:
                json_data = json.load(f)
    else:
    ...
    # shift the timetable by k_id minutes
    for i, time in enumerate(timetable):
        timetable[i] = time + dt.timedelta(minutes=k_id)
    return timetable
...
def get_timetables():
    for station in tqdm(list(station_dict.keys())[:-2]): # progress bar
    # for station in station_dict.keys()[:-2]:
                            ...
                            match station:
                                case "Edgware Road (Circle Line)":
                                    if next_stop == "Paddington":
                                        ...
                                        # next_stop_id = station_dict[next_stop]
                                        pad_hc_id = station_dict["Paddington (H&C Line)"]
                                        url = f"https://api.tfl.gov.uk/Line/{line}/Timetable/{pad_hc_id}/to/{station_id}"
                                        ...
                                        timetable = date_to_datetime(timetable)
                                        # add 2 minutes to each time
                                        for i, time in enumerate(timetable):
                                            timetable[i] = time + dt.timedelta(minutes=2)
                                        platform = Platform(station_objs[station], next_stop, line)
                                        platform.time_to_next_stop = time_to_next
                                        platform.departure_times = timetable
                                        platform.direction = direction
                                ...
                                case "Baker Street":
                                    if next_stop == "Edgware Road (Circle Line)":
                                        # next_stop_id = station_dict[next_stop]
                                        pad_hc_id = station_dict["Paddington (H&C Line)"]
                                        edgware_road_id = station_dict[next_stop]
                                        url = f"https://api.tfl.gov.uk/Line/{line}/Timetable/{edgware_road_id}/to/{pad_hc_id}"
                                        response = reqs.make_request(url)
                                        json_data = json.loads(response.read())
                                        timetable = [dt.time((int(train["hour"]) % 24), int(train["minute"])) for train in json_data["timetable"]["routes"][1]["schedules"][0]["knownJourneys"]]
                                        timetable = date_to_datetime(timetable)
                                        # subtract 2 minutes from each time
                                        for i, time in enumerate(timetable):
                                            timetable[i] = time - dt.timedelta(minutes=2)
                                        platform = Platform(station_objs[station], next_stop, line)
                                        platform.time_to_next_stop = time_to_next
        ...
        if ("district" in station_lines) and ("hammersmith-city" in station_lines) and ("circle" in station_lines):
            ...
            #region old
            # ups = [platform for platform in station_objs[station].platforms if (platform.direction == "up" and (platform.line == "bakerloo" or platform.line == "lioness"))]
            # downs = [platform for platform in station_objs[station].platforms if (platform.direction == "down" and (platform.line == "bakerloo" or platform.line == "lioness"))]
            # up_timetable1 = ups[0].departure_times
            # try:
            #     up_timetable2 = ups[1].departure_times
            # except IndexError:
            ...
            # big_platforms = station_objs[station].platforms
            # station_objs[station].platforms = [platform for platform in big_platforms if platform not in ups and platform not in downs]
            # up_platform = Platform(station_objs[station], up_next_stop, "bakerloo")
            # up_platform.time_to_next_stop = up_time_to_next
            # up_platform.departure_times = up_timetable
            # up_platform.direction = "up"
            # down_platform = Platform(station_objs[station], down_next_stop, "bakerloo")
            # down_platform.time_to_next_stop = down_time_to_next
            # down_platform.departure_times = down_timetable
            # down_platform.direction = "down"
            #endregion
...
if save and load:
    ...
    station_obj = station_objs["Kentish Town"]
    for platform in station_obj.platforms:
        if platform.next_stop == "Tufnell Park":
            platform.departure_times = [time - dt.timedelta(minutes=1) for time in platform.departure_times]

    station_obj = station_objs["Hammersmith (H&C Line)"]
    # add foot connection to Hammersmith (Dist&Picc Line)
    foot_platform = Platform(station_obj, "Hammersmith (Dist&Picc Line)", "foot")
    foot_platform.time_to_next_stop = 1
    foot_platform.departure_times = [dt.datetime.combine(dt.date.min, dt.time(4, 0)) + dt.timedelta(minutes=i) for i in range(60*21)]
else:
    station_objs = {}
    get_timetables()
...
# station = "Edgware Road (Circle Line)"
